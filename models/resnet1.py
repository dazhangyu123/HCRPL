...
def resnet50(args, **kwargs):
    ...
    if args.pretrained:
        if args.pretrained_checkpoint:
            ...
            pretrained_dict = {k: v for k, v in pretrained_dict_temp2.items() if k in model_dict.keys()}
            # temp = model_dict['fc.weight'].clone()
            # temp = pretrained_dict['fc.weight'].clone()
            # pretrained_dict['fc.weight'] = temp.clone()
            # temp = model_dict['fc.bias'].clone()
            # temp[:-1] = pretrained_dict['fc.bias'].clone()
...
def resnet101(args, **kwargs):
    ...
    # modify the structure of the model
    num_of_feature_map = model.fc.in_features
    model.fc = nn.Linear(num_of_feature_map, args.num_classes_s + 1)
    model_dict = model.state_dict()
    if args.pretrained:
        if args.pretrained_checkpoint:
            pretrained_dict_temp = torch.load(args.pretrained_checkpoint)['state_dict']
            ...
            pretrained_dict = {k: v for k, v in pretrained_dict_temp2.items() if k in model_dict.keys()}
            temp = model_dict['fc.weight'].clone()
            temp[:-1, :] = pretrained_dict['fc.weight'].clone()
            pretrained_dict['fc.weight'] = temp.clone()
            temp = model_dict['fc.bias'].clone()
            temp[:-1] = pretrained_dict['fc.bias'].clone()
            pretrained_dict['fc.bias'] = temp.clone()
...
def resnet152(args, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 8, 36, 3], args, **kwargs)
    # modify the structure of the model
    num_of_feature_map = model.fc.in_features
    model.fc = nn.Linear(num_of_feature_map, args.num_classes_s + 1)
    model_dict = model.state_dict()
